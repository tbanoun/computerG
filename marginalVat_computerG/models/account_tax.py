...
class AccountTax(models.Model):
    ...
    def _compute_taxes_for_single_line(self, base_line, handle_price_include=True, include_caba_tags=False,
        ...
        if taxes:
            # Calcul de la base taxable différente pour les produits d'occasion (Generate)
            if product and product.productSaleType == 'used':  # Utilisation du champ productSaleType
                # Récupérer le prix coûtant du produit
                ...
                taxes_res = taxes.with_context(
                    **base_line['extra_context'],
                    force_price_include=False,  # S'assurer que le calcul est fait sur la marge
                ).compute_all(
                    margin,
                    currency=currency,
                    ...
                    handle_price_include=False,  # Important pour le calcul sur marge
                    include_caba_tags=include_caba_tags,
                )

                # Ajuster les totaux pour afficher le prix complet mais taxe seulement sur marge
                taxes_res['total_excluded'] = price_unit_after_discount * base_line['quantity']
                taxes_res['total_included'] = taxes_res['total_excluded'] + sum(t['amount'] for t in taxes_res['taxes'])
            else:
                # Calcul normal pour les nouveaux produits (New)
            ...
            if early_pay_discount_computation == 'excluded':
                if product and product.productSaleType == 'used':
                    orig_margin = max(0, orig_price_unit_after_discount - product.standard_price)
