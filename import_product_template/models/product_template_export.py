    ...
    def exportData(self):
        ...
        for product in self:
            ...
            row = [
                ...
                "",  # x_product_website_url (empty in example)
                "",  # x_CPU (empty in example)
                "",  # x_sreen_size (empty in example)
                "",  # x_hddtype (empty in example)
                "",  # x_ram (empty in example)
                "",  # x_GPU (empty in example)
                "", # f"__export__.res_partner_{product.manufacturer.id}_{product.manufacturer.id}" if product.manufacturer else "",
                # manufacturer_id
                "",  # x_kind (empty in example)
                "",  # x_condition (empty in example)
                "",  # x_ (empty in example)
                "",  # image_url (you can add product.image_1920 URL here)
